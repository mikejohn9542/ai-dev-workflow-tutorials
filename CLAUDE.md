...
- **`data.py`** — pure pandas. No Streamlit or Plotly import. `load_sales_data(path)` reads and validates `data/sales-data.csv` (required columns present, non-empty, `date` parses to datetime, `quantity`/`unit_price`/`total_amount` are numeric with no missing/NaN values), raising `ValueError`/`FileNotFoundError` on failure. The rest of the module (`total_sales`, `total_orders`, `sales_by_month`, `sales_by_category`, `sales_by_region`) are calculations over the loaded DataFrame. `sales_by_category`/`sales_by_region` share a `_sales_by(df, column, dropna=False)` helper: rows with a null `category`/`region` are grouped under `"Unknown"` by default (so the breakdown chart's total still reconciles with `total_sales()`), and `app.py` offers a checkbox to pass `dropna=True` and exclude them instead. Being import-free of the UI layer is what makes every one of these independently unit-testable in `tests/test_data.py`.
...
- `is_numeric_dtype` alone isn't sufficient for numeric-column validation: pandas parses NA-like strings (e.g. `"N/A"`, `"NULL"`, `""`) straight to `NaN` during `read_csv`, which keeps the column numeric dtype but silently undercounts a KPI with no error. `load_sales_data` now also checks `df[col].isna().any()` for each numeric column and raises `ValueError` if any slipped through.
- `groupby` drops `NaN` group keys by default. `sales_by_category`/`sales_by_region` grouping directly on `category`/`region` would silently exclude rows with a missing dimension from the breakdown chart while `total_sales`/`total_orders` (which don't group on those columns) still counted them — making the chart totals quietly diverge from the KPI cards. Fixed by filling missing values with `"Unknown"` before grouping (default), with a `dropna=True` opt-out surfaced in the UI as a checkbox.
